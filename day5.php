...
while (!$codeZero) {
    // opcodes, last two digits
    $opcode = getOpcode(strval($code[$i]));
    // modes, from right to left
    $modes = array_map('intval', str_split(substr(strrev(strval($code[$i])), strlen($opcode) + 1)));
    switch ($opcode) {
        case 1:
        case 01:
            $value1 = getParam($modes[0] ?? 0, $i + 1, $code);
            $value2 = getParam($modes[1] ?? 0, $i + 2, $code);
            $result = $value1 + $value2;
            $code = setParam($modes[2] ?? 0, $i + 3, $result, $code);
            $program[] = [$modes, (string)$code[$i], $value1, $value2, $result, $i];
            $i += 4;
            break;
        case 2:
        case 02:
        ...
        case 03:
            $value1 = $code[$i + 1];
            $code[$value1] = $input;
            $program[] = [$code[$i], $value1, $input, $i];
            $i += 2;
            break;
        case 4:
        case 04:
            $result = getParam($modes[0], $i + 1, $code);
            if ($result === 0) {
                print ('Test successful, starting new test. . .<br>');
            } else {
                print ($result.'<br>');
            }
            $i += 2;
        ...
        case 07:
            ...
            $value2 = getParam($modes[1] ?? 0, $i + 2, $code);
            if ($value1 < $value2) {
                $code = setParam($modes[2] ?? 0, $i + 3, 1, $code);
            } else {
                $code = setParam($modes[2] ?? 0, $i + 3, 0, $code);
            }
            $i += 4;
            break;
        case 8:
...
function getParam($mode, $index, $code)
{
    switch ($mode) {
        case 0:
...
function getOpcode($input) {
    if (substr($input, -1) == '8') return 8;
    return intval(substr($input, -2));
}

function setParam($mode, $index, $value, $code)
{
    switch ($mode) {
        case 0:
