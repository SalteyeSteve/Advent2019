...
while (!$codeZero) {
    ...
    global $code;
    $opcode = intval(strrev((string)$code[$i])[0]);
    switch ($opcode) {
        case 1:
            // modes, from right to left
            $modes = array_map('intval', str_split(substr(strrev((string)$code[$i]), 2, strlen((string)$code[$i]))));
            $value1 = getParam($modes[0], $i + 1, $code);
            $value2 = getParam($modes[1], $i + 2, $code);
            $result = $value1 + $value2;
            $code = setParam($modes[2], $i + 3, $result, $code);
            $i += 4;
            break;
        case 2:
            // modes, from right to left
