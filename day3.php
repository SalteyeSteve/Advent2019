...
for ($i = 0; $i < count($leftWire); $i++) {
    ...
    var_dump($amount);
    $originalPosLeft = move($direction, $amount, $originalPosLeft);
    if ($originalPosRight === $originalPosLeft) {
        $intersectionArray[] = $originalPosLeft;
    }
    // right
    $direction = $rightWire[$i][0];
    $amount = (int)substr($rightWire[$i], 1);
    $originalPosRight = move($direction, $amount, $originalPosRight);
    if ($originalPosRight === $originalPosLeft) {
        $intersectionArray[] = $originalPosRight;
    }
}
...
//    echo $position[0].'x after<br>';
//    echo $position[1].'y after<br><br>';
   return $position;
}
